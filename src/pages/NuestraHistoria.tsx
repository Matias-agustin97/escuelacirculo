...
function NuestraHistoria() {
  ...
  const boxDesktop={display:"flex",justifyContent:"space-around",alignItems:"center",marginBottom:"3rem"}

  const boxMobile={display:"flex",flexDirection:"column"}

  const paraDesktop={flex:3,fontFamily:"Rubik",fontSize:"1.2rem",lineHeight:"1.5rem",padding:"1.5rem 2rem"}

  const paraMobile={fontSize:"0.95rem",lineHeight:"1.25rem"}

  const imgDesktop={flex:1,maxWidth:"60%",padding:"1.5rem"}

  const imgMobile ={height:"70vw",padding:"1rem"}
...
CÍRCULO DE PERIODISTAS DEPORTIVOS
    ...
    <Box sx={{display:matches? "flex":"block",flexDirection:"column"}}>
        <Box sx={matches? boxDesktop :boxMobile}><p style={matches?paraDesktop:paraMobile}>El 24 de mayo de 1941 se fundó el Círculo de Periodistas Deportivos. Hugo Marini (jefe de Deportes de Crítica), Salustiano González (jefe de Deportes de El Mundo), Emilio Rubio (de Noticias Gráficas), José López Pájaro (director de la Revista La Cancha), Alfredo Rossi (La Prensa), Ricardo Lorenzo "Borocotó" (Revista El Gráfico), Danilo Manzini (La Razón) e Idelfonso Talarúa Kendall (La Razón) firmaron el acta de fundación y convocaron a una Asamblea General para conformar la primera Comisión Directiva de la institución.
...
</p><img src={NuestraHistoria1} alt="" style={matches? imgDesktop:imgMobile}/></Box>
        <Box   sx={matches? boxDesktop :boxMobile}><p style={matches?paraDesktop:paraMobile}>En primera instancia, el Círculo de Periodistas Deportivos compartió la sede con el Círculo de la Prensa, ubicado en Rodríguez Peña 80. Allí se rubricó el acta fundacional y funcionó como casa del organismo por más de una década.
...
</p><img src={NuestraHistoria2} alt="" style={matches? imgDesktop:imgMobile} /></Box>
        <Box  sx={matches? boxDesktop :boxMobile}><p style={matches?paraDesktop:paraMobile}>El 1° de julio de 1960 se formalizó otro gran sueño: crear la primera escuela de periodismo deportivo de la Argentina, ubicada en el mismo edificio de la sede, a la que llamaron "José López Pájaro" en homenaje a uno de sus fundadores y hombre fundamental para llevar a cabo la iniciativa.
...
</p><img src={NuestraHistoria3} alt="" style={matches? imgDesktop:imgMobile} /></Box>
    </Box>
        </Container>
    </section>
  )
